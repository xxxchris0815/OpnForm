...
Then start OpnForm. `docker-compose.yml` must always be the first `-f` file. The Traefik overlay is not a complete Compose project on its own.

```bash
docker compose -f docker-compose.yml -f docker-compose.traefik.yml up -d
```

If your Compose version does not honor `ports: !override []`, comment out the `ports: - 80:80` mapping on `ingress` in `docker-compose.yml` so only Traefik is reachable from the internet.
...
The Traefik overlay only adds labels. It is not a complete Compose file. Always pass `docker-compose.yml` first:

```bash
docker compose -f docker-compose.yml -f docker-compose.traefik.yml -f docker-compose.override.yml up -d
```

Do not put an `ingress:` block in `docker-compose.override.yml` unless it also sets `image: nginx:1`.
</Accordion>
