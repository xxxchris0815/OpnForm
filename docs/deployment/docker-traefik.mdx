...
Wait until `api`, `ui`, and `ingress` are healthy. The first start can take a few minutes while images pull.
...
To deploy this checkout instead of the published Hub images:
...
Point `api` and `ui` at those tags with a `docker-compose.override.yml` as described in [Docker Deployment](/deployment/docker#building-custom-images).
