...
The repository includes `docker-compose.traefik.yml`. It repeats the full `ingress` service (including `image: nginx:1`) and adds Traefik labels. It does not use Compose `!reset` / `!override` tags, which some Compose versions treat as replacing the whole service.
...
Comment out the host port on `ingress` in `docker-compose.yml` so Traefik can bind 80/443:
...
`docker-compose.yml` must always be the first `-f` file. `docker-compose.override.yml` must not define `ingress`.
...
Do not add an `ingress` service to that file. Then start with every Compose file, in this order:

```bash
docker compose -f docker-compose.yml -f docker-compose.traefik.yml -f docker-compose.override.yml config
docker compose -f docker-compose.yml -f docker-compose.traefik.yml -f docker-compose.override.yml up -d
```

`config` must print `image: nginx:1` under `ingress` before you run `up`.
...
`docker-compose.override.yml` is merged last. If it contains an `ingress:` key without `image: nginx:1`, Compose drops the nginx image.

```bash
grep -n ingress docker-compose.override.yml
cp docker-compose.override.example.yml docker-compose.override.yml
```

Also comment out `ports: - 80:80` on `ingress` in `docker-compose.yml`. Then:
